...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function collectSkills() {
        return Array.from(document.querySelectorAll('.skill-tag')).map(function (tag) {
            return tag.textContent.replace('×', '').trim();
        }).filter(Boolean);
    }

    function collectLanguages() {
        return Array.from(document.querySelectorAll('.langue-ligne')).map(function (row) {
            const language = row.querySelector('.langue-select')?.value || '';
            const level = row.querySelector('.niveau-select')?.value || '';
    ...
    if (form) {
        form.addEventListener('submit', async function (e) {
            ...
            if (!email || !prenom || !nom) {
                alert('Veuillez remplir Email, Prenom et Nom.');
                return;
            }
